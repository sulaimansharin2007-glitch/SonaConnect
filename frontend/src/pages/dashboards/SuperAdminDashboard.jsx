...
import {
  Shield, Users, Calendar, CheckCircle, XCircle, Loader2,
...
export default function SuperAdminDashboard() {
  ...
  const defaultForm = {
    ...
    participationType: '', prizes: '', eligibility: '', clubName: '', club: '',
  };
                ...
                <motion.div key={ev._id} initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.05 }}
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <p className="text-white/40 text-xs mt-0.5">
                      {ev.category?.replace('_', ' ')} · {format(new Date(ev.date), 'dd MMM yyyy')} · by {ev.createdBy?.name || 'Unknown'}
                    </p>
                  </div>
