    ...
    <div className="min-h-screen bg-white bg-mesh">
      ...
      <div className="container-custom pt-24 pb-16">
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="mb-8">
          <h1 className="text-3xl font-black text-slate-900">Faculty Dashboard 👨‍🏫</h1>
          <p className="text-slate-700/50 mt-1">Manage workshops, hackathons and seminars</p>
        </motion.div>
        ...
        <div className="card overflow-hidden">
          <div className="p-5 border-b border-slate-900/10">
            <h3 className="font-bold text-slate-900">All Events ({myEvents.length})</h3>
          </div>
          <div className="overflow-x-auto">
            <table className="w-full text-sm">
              ...
              <tbody>
                {myEvents.length === 0 ? (
                  <tr>
                    <td colSpan="5" className="px-5 py-12 text-center text-slate-700/40">
                  ...
                  myEvents.map((ev) => (
                    <tr key={ev._id} className="border-b border-slate-900/5 hover:bg-white/[0.02]">
                      <td className="px-5 py-3 text-slate-900 font-medium max-w-48 truncate">{ev.title}</td>
                      ...
                      <td className="px-5 py-3">
                        <div className="flex items-center justify-end gap-2">
                          <button onClick={() => openEdit(ev)} className="p-1.5 rounded-lg text-slate-700/40 hover:text-blue-400 hover:bg-blue-500/10" title="Edit Event">
                            <Edit3 size={14} />
                          </button>
                          <button onClick={() => handleDelete(ev._id)} className="p-1.5 rounded-lg text-slate-700/40 hover:text-danger hover:bg-danger/10" title="Delete Event">
                            <Trash2 size={14} />
                          </button>
                        </div>
                      </td>
        ...
        {managedClubs.length > 0 && (
          <div className="mt-12 space-y-6">
            <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="mb-4">
              <h2 className="text-2xl font-black text-slate-900">Clubs under my In-charge 🏢</h2>
              <p className="text-slate-700/50 text-sm mt-1">View and manage members of the clubs you supervise</p>
            </motion.div>
              ...
              <div key={club._id} className="card p-6 space-y-4 bg-gradient-to-br from-primary-500/10 to-transparent border-primary-500/20">
                <div className="flex items-center justify-between border-b border-slate-900/10 pb-4">
                  <div>
                    ...
                    <p className="text-slate-700/40 text-sm mt-1">{club.description || 'No description provided.'}</p>
                  </div>
                  <div className="text-right">
                    <span className="text-2xl font-black text-slate-900">{club.members?.length || 0}</span>
                    <p className="text-slate-700/40 text-xs font-medium">Total Members</p>
                  </div>
                </div>
                ...
                <div>
                  ...
                  {(!club.members || club.members.length === 0) ? (
                    <p className="text-slate-700/40 text-sm">No members joined yet.</p>
                  ) : (
                    <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
                      {club.members.map((member) => (
                        <div key={member._id} className="glass p-4 rounded-2xl flex items-center gap-3 border border-slate-900/5 hover:border-slate-900/10 transition-all">
                          <div className="w-10 h-10 bg-gradient-primary rounded-xl flex items-center justify-center text-slate-900 font-bold text-sm">
                          ...
                          <div className="min-w-0 flex-1">
                            ...
                            <p className="text-slate-700/40 text-xs truncate">{member.email}</p>
